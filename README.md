...
**Run as a service (macOS):** Like [anchor_watch](../anchor_watch), you can run the web app under launchd so it starts at login and restarts if it exits. **`start-service.sh` forces production mode**, so you **must** put **`SECRET_KEY=...`** in **`float_plan/.env`** (same folder as `start-service.sh`). Without it, workers exit in a loop (`SECRET_KEY must be set` in **`data/service.error.log`**) and **`curl http://127.0.0.1:5503/`** hangs or returns nothing — even if the Cloudflare tunnel is correct. Generate: `python3 -c "import secrets; print(secrets.token_hex(32))"`. Casual **`./run_web.sh`** (no launchd) does not set `PRODUCTION`; you may omit **`SECRET_KEY`** there and rely on **`data/.flask_secret`**. Optionally set **`PORT`**, **`TRUST_PROXY=1`** behind Cloudflare, and **`RATE_LIMIT_STORAGE_URI`** / **`REDIS_URL`** if you run multiple Gunicorn workers. Run `./run_web.sh` once to create `.venv` and install dependencies, then:
